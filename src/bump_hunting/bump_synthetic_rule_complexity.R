...
rule_lengths = list(2,4,6,8,10)
amount = 0.2
min_value = 0
max_value = 1
num_rows = 10000
n_reps = 5
path = sprintf("../clean_code/results/rule_complexity/bh.csv")
#f = file(path,open="w")
write("n-rules,F1,F1-STD,Runtime,Runtime-STD",path)
for(j in 1:length(rule_lengths)) {
    
    f1_scores = rep(0, n_reps)
    runtimes = rep(0, n_reps)
    for(d in 1:n_reps){
        starting_time = Sys.time()
        rule_length = rule_lengths[[j]]
        data <- read.csv(paste0("../data/synthetic/rule_complexity/rule_complexity", rule_length,"_seed_",d-1,".csv"))
        X = data[,1:10]
        Y = data[,"Y"]
    ...
    print(sprintf("%d And-Clauses, %f F1", rule_length, mean_f1))
    path = sprintf("../clean_code/results/rule_complexity/bh.csv")
    write(sprintf("%s,%f,%f,%f,%f", rule_length, mean_f1, std_f1, mean_time, std_time),path,append=TRUE)
}