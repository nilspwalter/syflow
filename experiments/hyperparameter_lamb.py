...
for target_dist in config_exp.target_dist:
    ...
    path = base + "lamb_{}_{}.csv"
    with open(path.format(target_dist,args.method),"w") as f:
        writer = csv.writer(f)
        writer.writerow(["Lambda","F1","F1-STD","Runtime","Runtime-STD"])
        f.close()
    dim = config_exp.n_variables # is 10

    for lambd in config.lambds:
        ...
        with open(path.format(target_dist,args.method),"a") as f:
            writer = csv.writer(f)
            writer.writerow([lambd,f1,f1_std,runtime,runtime_std])
            f.close()
